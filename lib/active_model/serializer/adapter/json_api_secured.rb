    ...
    module Adapter
      class JsonApiSecured < Base
        ...
        def serializable_hash(options = nil)
          ...
          ApiObjects::JsonApiSecured.add!(hash)
        ...
        def fragment_cache(cached_hash, non_cached_hash)
          ...
          ActiveModel::Serializer::Adapter::JsonApiSecured::FragmentCache.new.fragment_cache(root, cached_hash, non_cached_hash)
        end
        ...
        def attributes_for(serializer, fields)
          all_attributes = serializer.attributes(fields).except(:id)
          allowed = AccessModel.attrs(serializer.object, :read)
          #Log.w fields
          #Log.w all_attributes
          #Log.w allowed
          all_attributes.select { |key,_| allowed.include? key }
        end
        ### TEMPIK END
        ...
        def relationships_for(serializer)
          ...
          serializer.associations(include_tree).each_with_object({}) do |association, hash|
            ass_obj = association.serializer && association.serializer.object
            if ['Array', 'ActiveRecord::Associations::CollectionProxy'].include? ass_obj.class.name
              ass_class = ass_obj.first.class.name
            else
              ass_class = ass_obj.class.name
            end
            Log.s "#{serializer.object.class.name} / #{association.key}"
            Log.s "ass_class: #{ass_class} ;  can read: #{AccessModel.model(ass_class, :read)} "
            next unless AccessModel.model(ass_class, :read)
